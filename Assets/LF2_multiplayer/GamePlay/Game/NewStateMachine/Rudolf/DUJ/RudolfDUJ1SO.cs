...
namespace LF2.Client{
    ...
    [CreateAssetMenu(fileName = "RudolfDUJ1", menuName = "StateLogic/Rudolf/Special/DUJ1")]
    public class RudolfDUJ1SO : StateLogicSO<RudolfDUJ1Logic>
    {
...
// Biscuit
    public class RudolfDUJ1Logic : LaunchProjectileLogic
    {
        ...
        public override void OnAnimEvent(int id)
        {
            if (stateData.SpawnsFX.Length == 0) {
                Debug.LogWarning($"You forgot set SFX for state {GetId()} Rudolf");
                return;
            }
            if (stateMachineFX.m_ClientVisual._IsServer) {
                SpwanProjectile(stateData.Projectiles[0], Vector3.right*stateMachineFX.CoreMovement.GetFacingDirection());
