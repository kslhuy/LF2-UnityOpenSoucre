...
namespace LF2.Server
{
    ...
    public class ServerGameNetPortal : MonoBehaviour
    {
        ...
        void ApprovalCheck(NetworkManager.ConnectionApprovalRequest request, NetworkManager.ConnectionApprovalResponse response)
        {
            ...
            {
                // Use m_Portal.AvatarRegistry.GetRandomAvatar instead of m_Portal.AvatarRegistry.GetSpecifiqueAvatar(0) when we finish Created Avatars  
                SessionManager<SessionPlayerData>.Instance.SetupConnectingPlayerSessionData(clientId, m_Portal.GetPlayerId(),
                    new SessionPlayerData(clientId, m_Portal.PlayerName, m_Portal.AvatarRegistry.GetSpecifiqueAvatar(0).Guid.ToNetworkGuid(),0, 0, true));
            ...
            if (gameReturnStatus == ConnectStatus.Success)
            {
                SessionManager<SessionPlayerData>.Instance.SetupConnectingPlayerSessionData(clientId, connectionPayload.playerId,
                    new SessionPlayerData(clientId, connectionPayload.playerName, m_Portal.AvatarRegistry.GetRandomAvatar().Guid.ToNetworkGuid(), 0,0, true));
