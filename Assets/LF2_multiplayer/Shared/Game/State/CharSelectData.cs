...
namespace LF2
{
    ...
    public class CharSelectData : NetworkBehaviour
    {
        ...
        private Dictionary<CharacterTypeEnum, Avatar> m_AvatarBOTByHero;

        public Dictionary<CharacterTypeEnum, Avatar> AvatarBOTByHero{
            get
                {
                    if( m_AvatarBOTByHero == null )
                    {
                        m_AvatarBOTByHero = new Dictionary<CharacterTypeEnum, Avatar>();
                        // co 1 list SkillsDescription o tren , lay tung cai 1 .
                        foreach (Avatar avatar in AvatarRegistry.m_Avatars)
                        {
                            if (m_AvatarBOTByHero.ContainsKey(avatar.CharacterClass.CharacterType))
                            {
                                throw new System.Exception($"Duplicate BOTChamp definition detected: {avatar.CharacterClass.CharacterType}");
                            }
                            m_AvatarBOTByHero[avatar.CharacterClass.CharacterType] = avatar;
                        }
                    }
                    return m_AvatarBOTByHero;
                }
        }
